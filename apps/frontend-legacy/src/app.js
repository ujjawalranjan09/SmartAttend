...
import { renderDailyPlan } from './views/daily-plan.js';
...
const NAV = {
  ...
  student: [
    ...
    { id: 'daily-plan', label: 'My Day', icon: 'calendar-clock' },
...
const VIEWS = { dashboard: renderDashboard, sessions: renderSessions, attendance: renderAttendance, students: renderStudents, analytics: renderAnalytics, reports: renderReports, settings: renderSettings, 'qr-scanner': renderQrScanner, profile: renderProfile, 'daily-plan': renderDailyPlan, 'forgot-password': (c, s) => import('./views/forgot-password.js').then(m => m.renderForgotPassword(c, s)), 'reset-password': (c, s) => import('./views/reset-password.js').then(m => m.renderResetPassword(c, s)) };
...
function setupAuthScreen() {
  ...
  form?.addEventListener('submit', async (e) => {
    ...
    try {
      ...
      const msg = err.message || (err.status === 429
