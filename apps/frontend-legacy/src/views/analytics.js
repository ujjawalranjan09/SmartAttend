...
function renderStudentAnalytics(container, data) {
    ...
    <div class="grid-2">
      ...
      <div class="card">
        <div class="card-header">
          <div><div class="card-title">Attendance Forecast</div><div class="card-subtitle">ML-powered 14-day prediction with confidence intervals</div></div>
          <div id="forecast-summary" style="font-size:var(--text-sm);text-align:right"></div>
        </div>
  ...
  const forecastDays = data?.forecast_7d || data?.forecast || actuals.slice(-3).map((v,i)=>v + (i+1)*0.5);
  // If we have structured forecast with confidence intervals from the ML service
  if (data?.forecast_trend && Array.isArray(data.forecast_trend) && data.forecast_trend.length > 0) {
    const ft = data.forecast_trend;
    const fLabels = ft.map((f,i) => `Day ${i+1}`);
    const predicted = ft.map(f => f.predicted_pct || f.pct);
    const upper = ft.map(f => f.upper_bound || f.predicted_pct + 5);
    const lower = ft.map(f => f.lower_bound || f.predicted_pct - 5);

    renderChart('s-forecast', 'line', fLabels, [
      { label: 'Upper bound', data: upper, borderColor: 'transparent', backgroundColor: 'rgba(109,170,69,0.1)', fill: false, pointRadius: 0 },
      { label: 'Forecast', data: predicted, borderColor: '#6daa45', borderDash: [5,5], tension: 0.3, pointRadius: 3, pointBackgroundColor: '#6daa45' },
      { label: 'Lower bound', data: lower, borderColor: 'transparent', backgroundColor: 'rgba(109,170,69,0.1)', fill: '-1', pointRadius: 0 },
    ]);

    // Show summary
    const next7d = data.forecast_7d_pct || data.next_7d_avg;
    const trend = data.forecast_trend_direction || data.trend_direction;
    ...
    document.getElementById('forecast-summary').innerHTML = `
      <div style="font-weight:600">${next7d ? `${next7d}%` : '--'} <span style="font-weight:400;font-size:var(--text-xs)">7-day avg</span></div>
      <div style="color:${trendColor}"><i data-lucide="${trendIcon}" style="width:14px;height:14px"></i> ${trend}</div>
    `;
    if (typeof lucide !== 'undefined') setTimeout(() => lucide.createIcons(), 50);
  } else {
    // Fallback: just show line
    const fLabels = Array.from({length: Math.min(forecastDays.length, 14)}, (_,i)=>`Day ${i+1}`);
    renderChart('s-forecast', 'line', fLabels, [{ label: 'Forecast', data: forecastDays.slice(0, 14), borderColor: '#6daa45', borderDash: [5,5], tension: 0.3 }]);
  }
