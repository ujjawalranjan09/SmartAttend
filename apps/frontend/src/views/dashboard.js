import { analyticsApi, sessionsApi } from '../utils/api.js';
...
async function renderFacultyDashboard(container, state) {
  let sessions = [];
  try { sessions = await sessionsApi.list({ limit: 5 }); } catch {}
  ...
  container.innerHTML = `
    ...
    <div class="kpi-grid">
      ...
      <div class="kpi-card"><div class="kpi-icon yellow"><i data-lucide="alert-triangle"></i></div><div><div class="kpi-value">--</div><div class="kpi-label">At-Risk Students</div></div></div>
    </div>
    <div class="card">
      ...
      <div class="table-wrapper">
        <table class="data-table">
          <thead><tr><th>Course</th><th>Date</th><th>Present</th><th>Status</th><th>Action</th></tr></thead>
          <tbody>
            ${sessions.length ? sessions.map(s => `
              <tr>
                ...
                <td><button class="btn btn-sm btn-secondary" onclick="document.getElementById('qr-modal-overlay').classList.remove('hidden')"><i data-lucide="qr-code"></i> QR</button></td>
              </tr>`).join('') : '<tr><td colspan="5" style="text-align:center;color:var(--color-text-muted);padding:2rem">No sessions yet</td></tr>'}
          </tbody>
        </table>
      </div>
    </div>`;
